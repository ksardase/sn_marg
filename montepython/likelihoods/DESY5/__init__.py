"""
.. module:: DESY5
    :synopsis: DESY5 likelihood based on arXiv:2401.02929 
               with analytical marginalization over the SN absolute magnitude
    :data: taken from https://github.com/CobayaSampler/sn_data/tree/master/DESY5
...
class DESY5(Likelihood_sn):
	...
	##This requires manipulating the covariance matrix.
        try:
            ...
            raise io_mp.LikelihoodError(
                "The DESY5 data files were not found. Please check if "
        ...
        np.fill_diagonal(self.C00, self.C00.diagonal() + self.light_curve_params.MUERR_FINAL**2)
	...
	#The data are ordered by increasing z, which simplifies things.
        self.true_size = len(self.light_curve_params.zHD)
...
#        covm += np.diag(sn.m_b_corr_err**2)
        ...
        print("DESY5 is initialized successfully")
